...
RESULTS_DIR="./results_LBALM_meps19"
PLOTS_DIR="./plots_LBALM_meps19"
...
print(f"Device: {DEVICE} | Method: LBALM | Dataset: MEPS Panel 19")
...
# ── Grid ──────────────────────────────────────────────────────────────────────
LBALM_GRID=[{"rho":rho,"lambda_init":li,"lr":lr,"t":t} for rho in [0.1,0.5,1.0] for li in [0.0,0.1] for lr in [1e-4,5e-4,1e-3] for t in [5.0,10.0,20.0]]
print(f"\nLBALM grid: {len(LBALM_GRID)} configs x {FAIRNESS_EPOCHS} epochs")
...
for SENSITIVE_COL in SENSITIVE_COLS:
    ...
    lbalm_rows=[]
    best_constraint_curve=None; best_lambda_curve=None; best_score_seen=-1e18
    ...
    for i,params in enumerate(LBALM_GRID):
        ...
        try:
            ...
            lam=torch.tensor(params["lambda_init"],device=DEVICE,dtype=torch.float32); rho=params["rho"]; t=params["t"]
            best_state,best_score=None,-1e18
            constraint_history=[]; lambda_history=[]
            ...
            row=make_row("LBALM",SENSITIVE_COL,params,te_m,va_m,B_val)
            lbalm_rows.append(row)
            feasible="✓" if va_m["loss"]-(B_val+DELTA)<=0 else "✗"
            print(f"  → acc={te_m['acc']:.4f} auc={te_m['auc']:.4f} p%-rule(sel)={te_m['p_rule_sel']:.4f} p%-rule(mp)={te_m['p_rule_mp']:.4f} val_c={va_m['loss']-(B_val+DELTA):+.4f} λ={float(lam.item()):.3f} {feasible}")
            if row["score"]>best_score_seen:
                best_score_seen=row["score"]
                best_constraint_curve=constraint_history; best_lambda_curve=lambda_history
        except Exception as e:
            print(f"  FAILED: {e}")


    csv_path=os.path.join(RESULTS_DIR,f"results_LBALM_{SENSITIVE_COL}.csv")
    pd.DataFrame(lbalm_rows).to_csv(csv_path,index=False)
    print(f"\nSaved {len(lbalm_rows)} rows → {csv_path}")
    all_sensitive_results[SENSITIVE_COL]={"lbalm_rows":lbalm_rows,"baseline_te":baseline_te,"B_val":B_val,"best_constraint_curve":best_constraint_curve,"best_lambda_curve":best_lambda_curve}

# ── Summary ───────────────────────────────────────────────────────────────────
print(f"\n{'='*70}\nSUMMARY\n{'='*70}")
for SENSITIVE_COL in SENSITIVE_COLS:
    if SENSITIVE_COL not in all_sensitive_results: continue
    data=all_sensitive_results[SENSITIVE_COL]; rows=data["lbalm_rows"]; bl_te=data["baseline_te"]
    ...
    print(f"  LBALM Best: acc={best['test_acc']:.4f} auc={best['test_auc']:.4f} p%-rule(sel)={best['test_p_rule_sel']:.4f} p%-rule(mp)={best['test_p_rule_mp']:.4f} val_c={best['val_constraint']:+.4f}")
...
for SENSITIVE_COL in SENSITIVE_COLS:
    ...
    ax.scatter(df_res.loc[fmask,"test_p_rule_sel"],df_res.loc[fmask,"test_acc"],
               color="purple",alpha=0.6,s=60,marker="D",label="LBALM feasible")
    ax.scatter(df_res.loc[~fmask,"test_p_rule_sel"],df_res.loc[~fmask,"test_acc"],
               facecolors="none",edgecolors="purple",alpha=0.5,s=60,marker="D",label="LBALM infeasible")
    ...
    ax.set_title(f"LBALM — MEPS Panel 19 — {SENSITIVE_COL}\nAll runs",fontsize=12)
    ...
    if c_curve:
        ...
        ax.plot(range(1,len(c_curve)+1),c_curve,color="purple",linewidth=2)
    ...
    if lam_curve and c_curve:
        ...
        ax1.plot(range(1,len(lam_curve)+1),lam_curve,color="purple",linewidth=2,label="λ")
        ax1.set_xlabel("Epoch",fontsize=12); ax1.set_ylabel("λ",color="purple",fontsize=12)
